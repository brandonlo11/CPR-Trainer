...
try:
    import pygame
    pygame.mixer.init()
    CLICK_WAV = pygame.mixer.Sound("click.wav")      # short tick file
except Exception:                                    # any audio problem ⇒ silent
    CLICK_WAV = None
...
while cap.isOpened():
    ...
    if cycle_active and resc_hand:
        ...
        if last_hand_y is not None:
            ...
            if dy > COMPRESSION_THRESHOLD and not is_compressing:
                ...
                is_compressing = False
                compressions_left -= 1
                if CLICK_WAV is None:                 # visual tick if no sound
                    metronome_flash = True
    ...
    cv2.putText(frame, sup_txt, (10,100), cv2.FONT_HERSHEY_DUPLEX, 1.3,
    ...
    cv2.putText(frame, posture_msg, (10,180), cv2.FONT_HERSHEY_DUPLEX, 1.3,
    ...
    if pre_count_active:
        ...
        cv2.putText(frame, "Cycle complete – reposition to restart", (10,260),
                    cv2.FONT_HERSHEY_DUPLEX, 1.3, INFO_COLOR, 2)
    ...
    if victim_pt and resc_hand and not hands_centered:
        cv2.arrowedLine(frame, resc_hand, victim_pt,
                        AR_ARROW_COLOR, 3, tipLength=0.25)
